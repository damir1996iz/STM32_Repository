...
#define LCD_DATA 1
//ADDITIONAL
#define CS_LOW GPIOA->ODR &=~GPIO_ODR_ODR4//SET CS(PA4) LOW (SPI1)
#define CS_HIGH GPIOA->ODR |=GPIO_ODR_ODR4//SET CS(PA4) HIGH (SPI1)

#define RESET_LOW GPIOC->ODR &=~GPIO_ODR_ODR14//SET RESET LOW
#define RESET_HIGH GPIOC->ODR |=GPIO_ODR_ODR14//SET RESET HIGH

#define A0_LOW GPIOA->ODR &=~GPIO_ODR_ODR3//SET A0 LOW(COMMAND)
#define A0_HIGH GPIOA->ODR |=GPIO_ODR_ODR3//SET A0 HIGH(DATA)
...
typedef struct 
{
	uint8_t x,y;//X,Y Coords of label
	
	uint16_t TextColor,BackgroundColor;//BorderColor
	
	uint8_t* txt;//Text on label
